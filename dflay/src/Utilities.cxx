...
namespace util_df {
  ...
  int LoadRunList(const char *inpath,const char *rfPrefix,std::vector<codaRun_t> &runList){
    ...
    std::string info;
    const int N = data.size(); // Number of unique runs in the .csv file
    std::vector<int> md;
    std::vector<pair<int, int>> segB_segE;
    for(int i=0;i<N;i++){
      aRun = std::atoi( data[i].c_str() );
      // rc = GetROOTFileMetaData(rfPrefix, aRun, md, segBeg, segEnd);
      rc = GetROOTFileMetaData(rfPrefix, aRun, md, segB_segE);
      ...
      crun.info         = info;
      // Keep record of all the beg and end segments for a particular run
      for (int i = 0; i < aNumFiles; i++) {
	crun.segmentBeg_End.push_back( make_pair(segB_segE[i].first, segB_segE[i].second) );
      }
  ...
  int LoadBCMData(std::vector<codaRun_t> runList, BCMManager *mgr){
    ...
    for(int i=0;i<NR;i++){
      ...
      //std::cout << " Total runs " << runList.size() << std::endl;
      if(NF==0){
	...
	for(int j=0;j<NF;j++){
	  ...
	  filePrefix = Form("%s/e1209019_fullreplay_%d_stream%d_seg%d_%d",
			    runList[i].rfPrefix.c_str(),runList[i].runNumber,runList[i].stream,runList[i].segmentBeg_End[j].first,runList[i].segmentBeg_End[j].second);
	  //if(j==0){
	  ...
	  // if (runList[i].runNumber == 13360) std::cout << " ** " << filePrefix << std::endl;
  ...
  int GetROOTFileMetaData(const char *rfDirPath, int run,
	...
	// if (theRun == 13360) {
	//   std::cout << " begSeg = " << bseg << " endSeg = " << eseg << endl; 
	//   std::cout << " *begSeg = " << segB_segE[fileCnt-1].first << " *endSeg = " << segB_segE[fileCnt-1].second << endl; 
	// }
	o2.clear();
      }
      o1.clear();
